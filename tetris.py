...
import pygame
from pygame.locals import QUIT, KEYDOWN, K_UP, K_LEFT, K_RIGHT, K_DOWN, K_ESCAPE
...
def is_overlapped(xpos,ypos,trun):
    data = BLOCK.type[trun]
    for y_offset in range(BLOCK.size):
        for x_offset in range(BLOCK.size):
...
def main():
    ...
    while True: # 게임 무한 루프
        ...
        if is_game_over():
            ...
            if erased > 0:
                score += (2**erased) * 100
                DIFFICULT = min(ceil(score/1000),30)
            BLOCK.draw()
